...
const PROGRAMS = [
  { id:1, category:"Board Exam Review", title:"Environmental Planning Board Examination Review",
    ...
    schedule:"Mon / Wed / Sat", fee:"₱18,500", fee_amount:18500, color:"#2E7D32",
        ...
        <div style={{ position:"relative", overflow:"hidden", borderRadius:20,
          ...
          <div style={{ position:"relative", zIndex:2 }}>
            <div style={{ width:120, height:120, borderRadius:"50%",
              ...
              display:"flex", alignItems:"center", justifyContent:"center",
              fontSize:48, boxShadow:"0 0 0 6px rgba(249,168,37,.35), 0 8px 32px rgba(0,0,0,.25)" }}>
              🏆
            </div>
            {/* Topnotcher badge */}
      ...
      <div style={{ maxWidth:1100, margin:"72px auto 0", padding:"0 20px" }}>
        ...
        <div style={{ background:"linear-gradient(135deg,#1B5E20,#2E7D32)",
          ...
          <div style={{ width:72, height:72, borderRadius:"50%",
            ...
            boxShadow:"0 0 0 4px rgba(249,168,37,.4)" }}>🏆</div>
          <div style={{ flex:1 }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))", gap:16 }}>
          {[
            { rank:2, name:"EnP Krisma Palado Alcantara",   initial:"KPA" },
            { rank:3, name:"EnP Leah Leopoldo Guarana",      initial:"LLG" },
            { rank:4, name:"EnP Angela Palanca Añonuevo",    initial:"APA" },
            { rank:5, name:"EnP Arvin Jay Manalo",           initial:"AJM" },
          ].map(p => (
            <div key={p.name} style={{ ...S.card,
              ...
              <div style={{ width:52, height:52, borderRadius:"50%", flexShrink:0,
                background:"linear-gradient(135deg,#2E7D32,#4CAF50)",
                display:"flex", alignItems:"center", justifyContent:"center",
                color:"#fff", fontWeight:800, fontSize:13 }}>{p.initial}</div>
              <div>
                <div style={{ fontSize:10, fontWeight:700, color:"#4CAF50",
                  textTransform:"uppercase", letterSpacing:".08em", marginBottom:3 }}>
                  2026 EPLEB Passer
                </div>
                <div style={{ fontWeight:800, fontSize:14, color:textMain, lineHeight:1.3 }}>
                  {p.name}
                </div>
        ...
        {portalTab==="dashboard" && (
          <div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
              <div style={S.card}>
                <h4 style={{ fontWeight:800, marginBottom:16, color:textMain }}>📢 Announcements</h4>
                {[["Enrollment Now Open","2026 batch enrollment is now open. Secure your slot today!"],
                  ["2026 Topnotcher!","EnP Collano Jr. — PLANNOVA's first reviewee topped the EPLEB!"],
                  ["AI Quiz now live","Try the AI-powered quiz generator in AI Tools"]].map(([t,d]) => (
                  <div key={t} style={{ padding:"10px 0", borderBottom:`1px solid ${border}` }}>
                    <div style={{ fontWeight:700, fontSize:14, color:textMain }}>{t}</div>
      ...
      <div style={{ maxWidth:1200, margin:"0 auto", padding:"36px 20px" }}>
        <div style={{ display:"flex", justifyContent:"space-between",
          ...
          <div style={{ display:"flex", gap:6, background:DM?"#1E3A20":"#F1F8F1",
            ...
            {["enrollments","students","finance","reports"].map(t => (
              <button key={t} onClick={() => setAdminTab(t)}
                style={{ background:adminTab===t?"#2E7D32":"transparent",
